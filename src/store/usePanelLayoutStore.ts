...
const defaultLayouts: Record<ViewId, ViewLayout> = {
  ...
  'vehicle-detail-photos': {
    panels: [{ id: 'photo-gallery', title: 'Fotos', icon: 'Camera', visible: true, order: 0 }],
  },
  'vehicle-detail-settings': {
    panels: [
    ...
    (set, get) => ({
      ...
      reorderPanels: (viewId, fromIndex, toIndex) => {
        set((state) => {
          ...
          console.log(
            'Updated panels:',
            updatedPanels.map((p) => ({ id: p.id, order: p.order, visible: p.visible }))
          );
          ...
          return {
            layouts: {
              ...
              [viewId]: {
                ...layout,
                panels: layout.panels.map((panel) =>
                  panel.id === panelId ? { ...panel, style: { ...panel.style, ...style } } : panel
                ),
              },
            },
          };
          ...
          return {
            layouts: {
              ...
              [viewId]: {
                ...layout,
                panels: layout.panels.map((panel) =>
                  panel.id === panelId ? { ...panel, style: undefined } : panel
                ),
              },
            },
          };
          ...
          return {
            layouts: {
              ...
              [toViewId]: {
                ...layout,
                panels: layout.panels.map((panel) =>
                  panel.id === toPanelId ? { ...panel, style: { ...fromStyle } } : panel
                ),
              },
            },
          };
