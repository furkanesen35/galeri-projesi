...
export const SalesWizard = ({ vehicle, onClose, onComplete }: Props) => {
  ...
  const [showAutoSignModal, setShowAutoSignModal] = useState<'buyer' | 'seller' | null>(null);
  ...
  const filteredClients = [...buyerProfiles, ...companyBuyerProfiles].filter((client) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="w-full max-w-5xl max-h-[90vh] bg-surface rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-gradient-to-r from-primary/5 to-transparent">
          <div>
            <h2 className="text-2xl font-bold text-foreground">Fahrzeug verkaufen</h2>
          ...
          {isTransitioning ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                ...
                <p className="text-sm text-text-secondary">Daten werden geladen...</p>
              </div>
            </div>
          ) : (
            <>
              ...
              {currentStep === 'terms' && <TermsStep data={formData} isLoading={isLoading} />}
              {currentStep === 'review' && (
                <ReviewStep
                  data={formData}
                  isLoading={isLoading}
                  onPreview={() => {
                    ...
                    console.log('📋 Current showDocumentPreview before:', showDocumentPreview);
                    setShowDocumentPreview(true);
                    console.log('✅ setShowDocumentPreview(true) called from Review');
                  }}
                />
              )}
      ...
      {(() => {
        console.log('🔎 Checking showDocumentPreview for modal render:', showDocumentPreview);
        return null;
      })()}
      {showDocumentPreview && (
...
const ClientSelectionStep = ({
  ...
  <div className="space-y-6">
    <div>
      <h3 className="text-lg font-semibold text-foreground mb-4">Kunde auswählen</h3>
              ...
              <div className="flex items-start gap-4">
                <div
                  className={`p-3 rounded-lg ${
                    isCompany ? 'bg-purple-500/20 text-purple-500' : 'bg-blue-500/20 text-blue-500'
                  }`}
                >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h4 className="font-semibold text-foreground truncate">
                      {isCompany ? (client as any).companyName : client.fullName}
                    </h4>
...
const BuyerInfoStep = ({
  ...
  <div className="space-y-6">
    <div>
      <h3 className="text-lg font-semibold text-foreground mb-4">Käuferinformationen</h3>
      <p className="text-sm text-text-secondary mb-6">
        {isLoading ? (
    ...
    <div className="grid grid-cols-2 gap-6">
      ...
      <div className="rounded-xl border border-border bg-bg-secondary/30 p-6">
        ...
        <div className="space-y-3">
          <FormInput
            label="Ausweisart"
            value={data.idType === 'personalausweis' ? 'Personalausweis' : data.idType}
            isLoading={isLoading}
...
const VehicleConfirmStep = ({
  ...
  <div className="space-y-6">
    <div>
      <h3 className="text-lg font-semibold text-foreground mb-4">Fahrzeugdaten bestätigen</h3>
      <p className="text-sm text-text-secondary mb-6">
        {isLoading ? (
          <span className="flex items-center gap-2">
            <div className="animate-spin h-4 w-4 border-2 border-primary border-t-transparent rounded-full" />
    ...
    {!isLoading && (
      <div className="rounded-lg bg-green-500/10 border border-green-500/20 p-4">
        <p className="text-sm text-foreground flex items-center gap-2">
          ...
          <span>Fahrzeug ist verkaufsbereit. Alle Dokumente sind vollständig.</span>
        </p>
      </div>
    )}
  </div>
...
const PricePaymentStep = ({
  ...
  <div className="space-y-6">
    <div>
      <h3 className="text-lg font-semibold text-foreground mb-4">Preis & Zahlungsbedingungen</h3>
      <p className="text-sm text-text-secondary mb-6">
        {isLoading ? (
          <span className="flex items-center gap-2">
            <div className="animate-spin h-4 w-4 border-2 border-primary border-t-transparent rounded-full" />
    ...
    <div className="grid grid-cols-2 gap-6">
      ...
      <div className="rounded-xl border border-border bg-bg-secondary/30 p-6">
        ...
        <div className="space-y-3">
          <FormInput
            label="Methode"
            value={data.paymentMethod === 'bank_transfer' ? 'Überweisung' : data.paymentMethod}
            isLoading={isLoading}
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
            <div>
              ...
              <p className="text-xs text-text-secondary">
                {isLoading ? '...' : new Date(data.paymentDueDate).toLocaleDateString('de-DE')}
              </p>
            </div>
            {isLoading ? (
              <SkeletonBar width="w-24" />
...
const TermsStep = ({ data, isLoading }: { data: typeof salesContractData; isLoading: boolean }) => (
  <div className="space-y-6">
    <div>
      <h3 className="text-lg font-semibold text-foreground mb-4">Vertragsbedingungen</h3>
      <p className="text-sm text-text-secondary mb-6">
        {isLoading ? (
          <span className="flex items-center gap-2">
            <div className="animate-spin h-4 w-4 border-2 border-primary border-t-transparent rounded-full" />
    ...
    <div className="grid grid-cols-2 gap-6">
      ...
      <div className="rounded-xl border border-border bg-bg-secondary/30 p-6">
        ...
        <div className="space-y-3">
          ...
          <InfoField label="Dauer" value={`${data.warranty.warrantyDuration} Monate`} />
      ...
      {/* Included Documents */}
      <div className="col-span-2 rounded-xl border border-border bg-bg-secondary/30 p-6">
        <h4 className="font-semibold text-foreground mb-4">Mitgegebene Dokumente</h4>
      ...
      <div className="col-span-2 rounded-xl border border-border bg-bg-secondary/30 p-6">
        <h4 className="font-semibold text-foreground mb-4">Weitere Konditionen</h4>
        <div className="grid grid-cols-3 gap-3">
          <ConditionBadge label="Probefahrt durchgeführt" active={data.terms.testDriveDone} />
          <ConditionBadge label="Fahrzeug begutachtet" active={data.terms.vehicleInspected} />
          ...
          <ConditionBadge label="Keine Inzahlungnahme" active={!data.terms.tradeIn} />
          <ConditionBadge label="Keine Finanzierung" active={!data.terms.financing} />
        </div>
      </div>
    </div>
  </div>
...
const ReviewStep = ({
  ...
  <div className="space-y-6">
    <div>
      <h3 className="text-lg font-semibold text-foreground mb-4">Vertragsübersicht</h3>
      <p className="text-sm text-text-secondary mb-6">
      ...
      <div className="rounded-xl border border-border bg-gradient-to-br from-blue-500/10 to-transparent p-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="font-semibold text-foreground">{data.buyer.fullName}</p>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-4">
      <ReviewSection title="Verkäufer (Autohaus)" icon={Building2}>
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <InfoField label="Adresse" value={data.seller.street + ', ' + data.seller.city} compact />
          <InfoField label="Vertreter" value={data.seller.representative} compact />
          <InfoField label="Position" value={data.seller.representativePosition} compact />
        </div>
      </ReviewSection>
      ...
      <ReviewSection title="Käufer" icon={User}>
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <InfoField label="Adresse" value={data.buyer.street + ', ' + data.buyer.city} compact />
      ...
      <ReviewSection title="Fahrzeug" icon={Car}>
        <div className="grid grid-cols-3 gap-3 text-sm">
          ...
          <InfoField label="Kennzeichen" value={data.vehicle.licensePlate} compact />
        </div>
      </ReviewSection>
      ...
      <ReviewSection title="Preis & Zahlung" icon={Euro}>
        <div className="grid grid-cols-4 gap-3 text-sm">
          ...
          <InfoField
            label="Fällig am"
            value={new Date(data.price.paymentDueDate).toLocaleDateString('de-DE')}
            compact
          />
        </div>
      </ReviewSection>
    ...
    <div className="rounded-lg bg-green-500/10 border border-green-500/20 p-6">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <h4 className="font-semibold text-foreground mb-2">Bereit zum Abschluss</h4>
          <p className="text-sm text-text-secondary mb-4">
            Alle Daten wurden überprüft. Mit Klick auf "Verkauf abschließen" wird der Kaufvertrag
            erstellt und das Fahrzeug als verkauft markiert.
          </p>
          <div className="flex items-center gap-2 text-xs text-text-secondary">
            ...
            <span>{new Date(data.contract.date).toLocaleDateString('de-DE')}</span>
          </div>
          {onPreview && (
            <button
              onClick={() => {
...
const DocumentSignatureStep = ({
  ...
  <div className="space-y-6">
    <div>
      <h3 className="text-lg font-semibold text-foreground mb-4">Kaufvertrag & Unterschriften</h3>
      <p className="text-sm text-text-secondary mb-6">
        Bitte unterschreiben Sie den Kaufvertrag digital. Der Vertrag wird automatisch generiert und
        gespeichert.
      </p>
    </div>
    ...
    <div className="rounded-xl border-2 border-primary/20 bg-gradient-to-br from-primary/5 to-transparent p-6">
      <div className="flex items-start gap-4 mb-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-text-secondary mb-4">
            Dieser Kaufvertrag wird nach Unterzeichnung automatisch als PDF generiert und beiden
            Parteien zugestellt.
          </p>
          <div className="grid grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-medium text-foreground">{data.buyer.fullName}</p>
            </div>
            <div>
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-text-secondary mt-1">{data.seller.representative}</p>
          </div>
          <button
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-text-secondary mt-1">{data.buyer.fullName}</p>
          </div>
          <button
    ...
    <div className="rounded-lg border border-border bg-bg-secondary/30 p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="text-sm text-text-secondary">
          <p className="font-medium text-foreground mb-1">Rechtlich verbindlich</p>
          <p>
            Mit Ihrer Unterschrift bestätigen Sie, dass Sie den Kaufvertrag gelesen haben und den
            Bedingungen zustimmen. Die digitale Unterschrift ist rechtlich bindend gemäß
            eIDAS-Verordnung.
          </p>
        </div>
      </div>
    ...
    {buyerSignature && sellerSignature && (
      <div className="rounded-lg bg-green-500/10 border border-green-500/20 p-4">
        <p className="text-sm text-foreground flex items-center gap-2">
          <CheckCircle2 className="h-4 w-4 text-green-500" />
          <span>Alle Unterschriften vollständig. Sie können den Verkauf nun abschließen.</span>
        </p>
      </div>
    )}
  </div>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-[21cm] max-h-[95vh] bg-white rounded-lg shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b-2 border-gray-300 bg-gray-50 print:hidden">
          <div>
            <h3 className="text-lg font-bold text-gray-900">Kaufvertrag - Vorschau</h3>
            <p className="text-xs text-gray-600 mt-1">Druckfertiges A4-Format</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
        ...
        <div className="flex-1 overflow-y-auto bg-gray-100 p-8">
          ...
          >
            ...
            <div className="text-center mb-12 pb-6 border-b-4 border-gray-900">
              ...
              <h1 className="text-4xl font-bold text-gray-900 mb-2">KAUFVERTRAG</h1>
              <p className="text-lg text-gray-700">über ein gebrauchtes Kraftfahrzeug</p>
              <p className="text-sm text-gray-600 mt-4 font-mono">
                Vertragsnummer: {data.contract.contractNumber}
              </p>
              <p className="text-sm text-gray-600 font-mono">
                Datum: {new Date(data.contract.date).toLocaleDateString('de-DE')}
              </p>
            </div>
              ...
              <div className="grid grid-cols-2 gap-8">
                ...
                <div className="border-2 border-gray-900 p-5 bg-gray-50">
                  ...
                  <div className="space-y-2 text-sm">
                    <p className="font-bold text-base text-gray-900">{data.seller.name}</p>
                    ...
                    <div className="pt-3 mt-3 border-t border-gray-400">
                      ...
                      <p className="text-gray-700">E-Mail: {data.seller.email}</p>
                      <p className="text-gray-700 mt-2">
                        USt-IdNr: <strong>{data.seller.ustId}</strong>
                      </p>
                      <p className="text-gray-700">{data.seller.commercialRegister}</p>
                    </div>
                    <div className="pt-3 mt-3 border-t border-gray-400">
                      <p className="text-gray-700">Vertreten durch:</p>
                      <p className="font-semibold text-gray-900">{data.seller.representative}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-2 border-gray-900 p-5">
                  ...
                  <div className="space-y-2 text-sm">
                    <p className="font-bold text-base text-gray-900">{data.buyer.fullName}</p>
                    <p className="text-gray-800">
                      Geb.: {new Date(data.buyer.dateOfBirth).toLocaleDateString('de-DE')}
                    </p>
                    ...
                    <div className="pt-3 mt-3 border-t border-gray-400">
                      ...
                      <p className="text-gray-700">E-Mail: {data.buyer.email}</p>
                      <p className="text-gray-700 mt-2">Ausweis: {data.buyer.idNumber}</p>
              ...
              <table className="w-full border-2 border-gray-900">
                <tbody>
                  <tr className="bg-gray-900 text-white">
                    <td className="border border-gray-700 px-4 py-3 font-bold" colSpan={2}>
                      FAHRZEUGDATEN
                    </td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td className="border border-gray-400 px-4 py-2 text-gray-900 bg-white">
                      {new Date(data.vehicle.firstRegistration).toLocaleDateString('de-DE')}
                    </td>
                  </tr>
                  <tr>
                    <td className="border border-gray-400 px-4 py-2 font-semibold bg-gray-100 text-gray-900">
            ...
            <div className="mb-10">
                ...
                <table className="w-full">
                  <tbody>
                    ...
                    <tr className="border-t-4 border-gray-900">
                      <td className="py-3 text-xl font-bold text-gray-900">GESAMTKAUFPREIS:</td>
              ...
              <div className="mt-6 p-4 border-2 border-gray-400 bg-white">
                <h3 className="font-bold text-gray-900 mb-3">Zahlungsmodalitäten:</h3>
                <ul className="space-y-2 text-sm text-gray-900">
                  <li className="flex justify-between">
                    <span className="text-gray-900">• Anzahlung bei Vertragsunterzeichnung:</span>
                  ...
                  <li className="flex justify-between">
                    <span className="text-gray-900">
                      ...
                      {new Date(data.price.paymentDueDate).toLocaleDateString('de-DE')}:
                    </span>
                    <strong className="text-gray-900">
            ...
            <div className="mb-10">
              ...
              <div className="text-sm space-y-2 p-4 border-2 border-gray-400 bg-white">
                <p className="text-gray-900">
                  <strong className="text-gray-900">Art der Gewährleistung:</strong>{' '}
                ...
                <p className="text-gray-900">
                  <strong className="text-gray-900">Umfang:</strong> {data.warranty.warrantyScope}
                </p>
                <p className="text-gray-900">
                  <strong className="text-gray-900">Kilometerbegrenzung:</strong>{' '}
            ...
            <div className="mb-10">
              ...
              <div className="text-sm p-4 border-2 border-gray-400 bg-white">
                ...
                <p className="mb-4 text-gray-900">
                  <strong className="text-gray-900">Übergabeort:</strong> {data.handover.location}
                </p>
              ...
              <div className="grid grid-cols-2 gap-12">
                  ...
                  <div className="pt-16 border-t-2 border-gray-900 mt-20">
                    {sellerSignature && (
                      <div className="mb-4 -mt-20">
                        <img src={sellerSignature} alt="Unterschrift Verkäufer" className="h-16" />
                      </div>
                    )}
                    <p className="font-bold text-gray-900">Unterschrift Verkäufer</p>
                    <p className="text-sm text-gray-700 mt-1">{data.seller.representative}</p>
                    <p className="text-xs text-gray-600">{data.seller.representativePosition}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="pt-16 border-t-2 border-gray-900 mt-20">
                    {buyerSignature && (
                      <div className="mb-4 -mt-20">
                        <img src={buyerSignature} alt="Unterschrift Käufer" className="h-16" />
                      </div>
                    )}
                    <p className="font-bold text-gray-900">Unterschrift Käufer</p>
                    <p className="text-sm text-gray-700 mt-1">{data.buyer.fullName}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-12 p-6 bg-gray-100 border-2 border-gray-400 text-xs text-gray-700 leading-relaxed">
              <p className="font-bold text-sm text-gray-900 mb-3">RECHTLICHE HINWEISE:</p>
              <p className="mb-2">
                Der Verkäufer bestätigt, dass er Eigentümer des oben beschriebenen Fahrzeugs ist und
                dieses frei von Rechten Dritter ist. Das Fahrzeug wird unter Ausschluss jeglicher
                Sachmängelhaftung verkauft, soweit gesetzlich zulässig.
              </p>
              <p className="mb-2">
                Der Käufer bestätigt, das Fahrzeug besichtigt und Probe gefahren zu haben. Der
                Zustand des Fahrzeugs wurde zur Kenntnis genommen und akzeptiert. Mündliche
                Nebenabreden bestehen nicht.
              </p>
              <p>
                Dieser Kaufvertrag wurde in zweifacher Ausfertigung erstellt. Jede Vertragspartei
                erhält eine Ausfertigung. Die digitalen Unterschriften sind gemäß eIDAS-Verordnung
                (EU) Nr. 910/2014 rechtlich bindend.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-start gap-4 mb-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-text-secondary">
              ...
              {party === 'buyer' ? 'des Käufers' : 'des Verkäufers'} automatisch laden und
              verwenden?
            </p>
          </div>
        </div>
...
const SkeletonBar = ({ width = 'w-full', height = 'h-9' }: { width?: string; height?: string }) => (
...
const InfoField = ({
  ...
  <div className={compact ? 'space-y-1' : 'space-y-2'}>
    ...
    <div className="flex items-center gap-2">
      ...
      <p className={`${compact ? 'text-sm' : 'text-base'} font-medium text-foreground`}>{value}</p>
    </div>
  </div>
);
